...
- **Scraper `HttpClient` Socket Exhaustion**: Introduced `InkTag.Core.Net.SharedHttpClient`, a process-wide pooled `HttpClient` (`SocketsHttpHandler`, 2-minute `PooledConnectionLifetime`). `RateLimitedHttpClient`, `MetadataScraperService` cover-thumbnail downloads, and `BulkScrapeQueueService` now use it instead of allocating a new `HttpClient` per candidate / per service instance, which under a large bulk scrape could exhaust sockets and ephemeral ports. Per-request time limits are enforced with linked `CancellationTokenSource`s.
- **Scraper Cache Lost on One-Shot Runs**: `MetadataScraperService` and `ComicVineProvider` are now `IDisposable` and flush the ComicVine response cache synchronously on dispose. Previously the 7-day disk cache was only written by a 2-second debounce timer, so a single `inktag scrape` command or MCP scrape tool call exited before the write fired and never persisted anything. The CLI `scrape` command and the `search_external_metadata`, `scrape_comic_metadata`, and `bulk_scrape_directory` MCP tools now dispose the service.
...
- **README Accuracy Pass**: Corrected inaccurate claims in `README.md` — test count (94 → 211), required SDK (`10.0 / 9.0 / 8.0` → `10.0` only, matching the `net10.0` target), a non-existent `Ctrl+M` hotkey, "reading and writing `.cbr`" (writes always repack to `.cbz`), the missing `remove_comic_page` / `strip-intro` / `remove-page` commands, MCP tool parameter names (`filePath`/`directoryPath` → `path`/`directory`), the "atomically" claim on `restore_batch_job` ([#21](https://github.com/aurora7795/InkTag/issues/21)), and the `check_komga_server` / `audit_komga_library` descriptions. Also dropped "Komga" from the *Upcoming* roadmap (already shipped in v0.11.1).
- **Wiki Accuracy Pass** (`docs/wiki/`, `.agents/skills/comic-metadata-curator/SKILL.md`): Reconciled the Layer 2 wiki with the current code — test count (184 → 211), MCP tool count (14 → 18, with the missing `remove_comic_page` / `check_komga_server` / `sync_komga_book_or_series` / `audit_komga_library` entries added), stale `MetadataBackupService` / `PerceptualHashService` / `KomgaSyncService` / cover-extraction method signatures, non-existent GUI ViewModel names (`ScraperMatchViewModel` etc.), the `LruImageCache` "auto-disposes evicted bitmaps" claim (disposal was removed in v0.13.0), `restore_comic_backup` parameter (`timestamp` → `backupId`), the "atomic" batch-rollback wording, and the missing `strip-intro` / `remove-page` CLI subcommands.
- **Scrape Applied Metadata Twice with Divergent Merge Modes**: `MetadataScraperService.AutoScrapeComicAsync` no longer mutates the caller's `ComicInfo`. It returns the raw fetched issue metadata on the new `ScrapeResult.FetchedMetadata`, and the single `ApplyMetadata` call at write time owns the merge mode. Previously `AutoScrapeComicAsync` pre-applied the fetched data under `Settings.DefaultMergeMode`, then the CLI `scrape` command / MCP `scrape_comic_metadata` tool re-applied the already-merged object under the user's `mode`, so `mode: "overwrite"` was partially ignored and `<Notes>` attribution could be merged twice. The unused `ScrapeResult.TargetComic` property was removed.
