...
### Phase 1: Directory Operations & Scanner UX
1. **Open Directory**: Click *Open Folder*, pick a path with mixed folders and archives. Check that scanning runs in parallel on background threads with real-time numeric progress (`Scanning: X/Y files`).
2. **Cancel Scan**: Click `✕ Cancel` during a long scan. Confirm the scan stops immediately and retains whatever files were already processed.
3. **Recursive Scan**: Toggle the recursive scan checkbox, reload, and verify that archives located inside subfolders populate correctly.

### Phase 2: Metadata Editing & Scraping
...
3. **Series Search Wizard**: Click *Series Wizard*, search for a volume (e.g. "100 Bullets"), verify publisher badges and description snippets display cleanly.
...
### Phase 4: Save & Safety
1. **Save Batch**: Click *Save All*. Verify saving runs in the background while updating the progress bar.
